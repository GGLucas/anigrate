...
<section id='about'>
  ...
  there's no need to configure or set anything up other than the main application. 
...
<section id="install">
  ...
  <p>You can also clone the git repository and run <code>make install</code>.
  </p>
...
<section id="usage">
  ...
  <p>Running just <code>$ anigrate watch</code> will display a matcher menu 
  where you can type a partial name and select the series you want to add a log 
  entry for.  Since you did not specify a count, it will assume you watched a 
  single episode.</p>
...
<section id="selectors">
  ...
  <h2>matcher</h2>
   <p>Instead of immediately evaluating the selector, show a matcher window
   displaying all series that match and letting you choose a single one.</p>
